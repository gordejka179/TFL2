...
Заметим, что исходная регулярка не допускает строк, которые содержат в качестве подстроки "ecccccS", где первый символ строки S отличен от "c". Объяснение: заметим, что 2 и более буквы "c" подряд могут идти в следующих ситуациях: 
- `dcdc*c(d*|e*e)`.  
Но тут перед c*c идёт буква "d".
- `e(cc)*(ee|d)` (из-за звездочки). 
Но тогда длина (cc)^n будет чётной.
- если сконкатенировать `caс` с `c(a|c|d|e)`.   
Но тут перед буквой "c" идёт буква "a".

ПКА1 представлен на картинке pka.jpg
![МалПКА](./img/pka.jpg)

Таблица:

|   | a | $\varepsilon$ | c | cc | ccc | cdcc |
|---|:---:|:--:|:--:|:--:|:--:|:--:|
| dcdccc | - | + | + | + | + | + |
| dcdcc | . | - | + | + | + | + |
| dcdc | . | . | - | + | + | + |
...
Заметим, что все буквы "a" кроме последней (имеется в виду если слово заканчивается на "ca") должны быть в конструкции "cac".  
ПКА2 представлен на картинке pka2.jpg
![МалПКА](./img/pka2.jpg)

# Расширенное регулярное выражение

`^(dcdc+(d*|e+)|e(cc)*(ee|d)|cac|dd)*c.$`  
Была идея добавить предпросмотр на наличие одной из комбинаций "ca", "cc", "cd", "ce".
И для ПКА читать блок `(dcdc*c(d*|e*e)|e(cc)*(ee|d)|cac|dd)` , но это показалось слишком глупо, потому что такая комбинация нужна лишь в конце строки.
